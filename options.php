...
defined('ADMIN_MODULE_NAME') or define('ADMIN_MODULE_NAME', 'afonya_newslog');
...
if ((!empty($save) || !empty($restore)) && $request->isPost() && check_bitrix_sessid()) {
    if (!empty($restore)) {
    ...
    } elseif ($request->getPost('active') || $request->getPost('iblock_type') || $request->getPost(
            'time'
        ) || $request->getPost('email') || $request->getPost('EventMessageId')) {
        $active = "N";
        ...
        if ($AgentId > 0) {
            CAgent::Update(
                $AgentId,
                array('ACTIVE' => $active, 'AGENT_INTERVAL' => 60 * 60 * 24 * $request->getPost('time'))
            );
        }
    ...
    <tr>
        ...
        <td width="60%">
            ...
            echo CAdminCalendar::CalendarDate(
                "date",
                Option::get(ADMIN_MODULE_NAME, "date", date('Y-m-d H:i:s')),
                20,
                true
            ) ?>
        </td>
